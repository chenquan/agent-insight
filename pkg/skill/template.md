---
name: agent-insight
description: "使用 agent-insight 分析 pprof 性能 profile 文件。TRIGGER 当: 用户提到 pprof、性能分析、CPU profiling、内存泄漏、heap 分析、性能热点、火焰图、.pb.gz 文件、goroutine 泄漏、延迟分析、概况、元信息、调用链、执行路径、调用树、层级结构、pprof 标签、label、tag、goroutine state、http method、或想对比 profile 性能差异、或想知道样本按什么分组。"
---
...
| 用户提到 pprof 标签 / goroutine state / http.method / 想知道样本按什么分组 | `agent-insight tags profile.pb.gz` |
| 用户想按 pprof label 过滤热点（如只看 state=blocked） | `agent-insight analyze/list/traces/diff ... --tag key=value` |
...
```bash
agent-insight tags <profile.pb.gz> [flags]
```

| Flag | 默认值 | 说明 |
|------|--------|------|
| `--top N` | 50 | 每个 numeric label 最多展示 N 个 value（string label 不截断） |
| `--format` | text | 输出格式: text, json, markdown |

发现层：列出 profile 中所有 pprof 标签（Sample.Label）及其 value 分布。在做 `--tag` 过滤或 breakdown 前先跑 `tags` 了解有哪些标签维度。
...
| `--tag key=value` | | 按 pprof label 过滤样本（可重复；同 key OR，跨 key AND） |
| `--tag-ignore key=value` | | 排除匹配 label 的样本（同 --tag 语义） |
| `--tag-breakdown-on k1,k2` | | 展开指定 label key 的每函数 flat 分布 |
| `--tag-breakdown-top N` | 20 | 展开前 N 个函数的 label 分布 |
...
| `--ignore-function pattern` | | 正则排除函数（原 --exclude，已改名） |
| `--tag key=value` | | 按 pprof label 过滤样本（可重复；同 key OR，跨 key AND） |
| `--tag-ignore key=value` | | 排除匹配 label 的样本（同 --tag 语义，不接受正则） |
| `--format` | text | 输出格式 |
| `--value-type` | | 指定值类型 |
...
| `--focus pattern` | | 正则过滤函数 |
| `--ignore pattern` | | 正则排除函数 |
| `--tag key=value` | | 按 pprof label 过滤样本，base 和 target 同 filter（可重复；同 key OR，跨 key AND） |
| `--tag-ignore key=value` | | 排除匹配 label 的样本（同 --tag 语义） |
| `--format` | text | 输出格式 |
...
```
用户说"只看阻塞的 goroutine" / "哪个端点慢" / "按 http.method 分组"
  │
  1. agent-insight tags profile.pb.gz --format json
  │   → 先看 profile 有哪些 label（state、wait_reason、http.method…）及 value 分布
  │
  2. agent-insight analyze profile.pb.gz --tag state=blocked --format json
  │   → 只分析匹配标签的样本（goroutine 阻塞态）
  │
  3. agent-insight analyze profile.pb.gz --tag-breakdown-on http.method --format json
      → 展开每个热点函数在各 label value 上的 flat 分布
```

**决策点 (步骤 1)**:
- `tags` 输出 `Labels: 0 keys` → 该 profile 无标签，`--tag` 过滤会报 "matched 0 of N samples"，改用 analyze 直接分析
- `state` 有 blocked/running/syscall 等枚举值 → goroutine 阻塞分析用 `--tag state=blocked`
- `http.method` 有 GET/POST… → 按端点拆分用 `--tag-breakdown-on http.method`

**决策点 (步骤 3)**:
- breakdown 显示某函数 state=blocked 占 80% flat → 该函数耗时几乎全在阻塞态，重点查锁/IO
- breakdown 各 value 分布均匀 → 该函数无特定 label 偏好，标签过滤增益有限

**陷阱提示**:
- `--tag key=value` 同 key 多次是 OR，跨 key 是 AND；`--tag-ignore` 反向（排除）
- `--tag-breakdown-on` 只在指定 key 时计算；不传则不输出 label_breakdown（默认行为不变）
- breakdown 的 cum 字段在 v1 恒为 0（保留字段），只看 flat 分布
...
| `info` 显示 `Labels: N keys`（N>0） | `tags` 看详细 label 分布，再决定是否 `--tag` 过滤 |
| goroutine profile 想看阻塞态 goroutine | `analyze goroutine.pb.gz --tag state=blocked` |
...
| `label_summary` | 标签概览（`key_count` / `distinct_values`），无标签时为 0/0 |
...
| `type` | profile 类型 |
| `total_samples` | 采样总数 |
| `labels[].key` | 标签键（如 state、http.method） |
| `labels[].type` | `string` 或 `numeric` |
| `labels[].unit` | numeric label 的单位（string 为 null） |
| `labels[].distinct` | 该 key 的不同 value 总数 |
| `labels[].values` | value 分布（按 count 降序），每项含 value/count/percent |
| `labels[].values_truncated` | 是否因 `--top` 截断 |
...
| `file` | 源文件路径 |
| `line` | 行号 |
| `label_breakdown` | 该函数在各 label value 上的 flat 分布（仅 `--tag-breakdown-on` 时输出，含 key/values/flat/flat_pct，v1 cum 恒为 0） |
...
- `list` 命令的 `--exclude` 已改名为 `--ignore-function`（仅函数名正则排除）；新增 `--tag-ignore key=value` 用于 label 过滤，两者完全独立
